public class Main {
    ...
    public static int numberOfVowels(String str){
        ...
        for (int i = 0; i < str.length(); i++) {
            ...
            if(temp.equals("a") || temp.equals("e") || temp.equals("i") || temp.equals("o") || temp.equals("u")) count ++;
        }
        return count;
    }
    ...
    public static String camelCaseMe(String str) {
        ...
        for (int i = 0; i < str.length(); i++) {
            if (str.charAt(i) == '-' || str.charAt(i) == '_' && (str.charAt(i) == 0)) {
                String upperChar = str.substring(0, 1).toUpperCase();
                int end = 0;
                for (int h = 1; h < str.length(); h++) {
                    if (str.charAt(h) == '-' || str.charAt(h) == '_') end = h;
                }
                String tempWord = str.substring(1, end);
            }
            else if ((str.charAt(i) != 0)) {
                String upperChar = str.substring(startIndex, startIndex+1).toUpperCase();
                String tempWord = str.substring(startIndex+1, i);
                result = result + upperChar + tempWord;
            }
